...
function Schroedinger(settings,s){
	...
	this.plotPot       = new Array(s.width);
	this.plotX         = new Array(s.width);
	this.plotP         = new Array(s.width);
	...
	this.show = function(){
		...
		let textHeight = s.height/30;
		let topMargin  = 1.5*textHeight;
		let heightX    = 0.7*s.height;
		let seperator  = heightX + topMargin + 2;
		let heightP    = s.height - 2*topMargin - heightX - 2;
		...
		this.plotComplexData(this.psiP, this.plotP, heightP, s.height - heightP, 1/this.maxFourierAmp);
	...
	this.drawUnderlay = function(seperatorHeight, plotHeight, topMargin){
		if(this.frameCount == 0){
			...
			let delta                  = 0.5*s.width*maxPotEnergyWaveVector/maxWaveVector*this.momentumZoom;
			
			this.underlay.stroke(235);
			this.underlay.strokeWeight(3);
			this.underlay.line(0,seperatorHeight,s.width,seperatorHeight);
			this.underlay.strokeWeight(1);
			this.underlay.fill(100);
			this.underlay.rect(0.5*s.width - delta,seperatorHeight, 2*delta, s.height-seperatorHeight);
			this.underlay.line(0.5*s.width, seperatorHeight, 0.5*s.width, s.height); 
			...
			this.underlay.vertex(s.width-1,y);
			this.underlay.vertex(s.width-1,seperatorHeight-1);
			this.underlay.endShape(s.CLOSE);
		}
		
		s.image(this.underlay,0,0);
	}
	...
	this.plotComplexData = function(data, plotMap, plotHeight, topMargin, scaleFactor){
		...
		for(let j = 1; j <s. width-1; j++){
			...
			s.stroke(cl,225,235);
			s.line(j,plotHeight + topMargin,j,y);
		}
	}
	...
	this.drawOverlay = function(seperatorHeight, textHeight){
		...
		s.text('Position Space', 0.25*textHeight, 0.25*textHeight);
		s.text('Momentum Space (x' + this.momentumZoom + ')', 0.25*textHeight, seperatorHeight + 0.25*textHeight);
		s.textAlign(s.RIGHT,s.TOP);
		s.text('P(x<.5) = ' + this.statistics.leftX.toFixed(3), 0.41*s.width, 0.25*textHeight);
		s.text('P(p<0) = ' + this.statistics.leftP.toFixed(3), 0.41*s.width, seperatorHeight + 0.25*textHeight);
		s.textAlign(s.CENTER,s.TOP);
		s.text('<x> = ' + this.statistics.meanX.toPrecision(3), 0.5*s.width, 0.25*textHeight);
		s.text('<p> = ' + this.statistics.meanP.toPrecision(3), 0.5*s.width, seperatorHeight + 0.25*textHeight);
		s.textAlign(s.LEFT,s.TOP);
		s.text('RMS(x) = ' + this.statistics.rmsX.toPrecision(3), 0.59*s.width, 0.25*textHeight);
		s.text('RMS(p) = ' + this.statistics.rmsP.toPrecision(3), 0.59*s.width, seperatorHeight + 0.25*textHeight);
		s.textAlign(s.RIGHT,s.TOP);
		s.text('t = ' + this.statistics.time.toFixed(5), s.width - 0.25*textHeight, 0.25*textHeight);
		s.text(this.label, s.width - 0.25*textHeight, seperatorHeight + 0.25*textHeight);
	}
